...
#define THREADNUM3 4    // outTrans线程数，定死
#define DEGREE 50		// 入度出度的值
...
//哈希表实现
#define HASHSIZE 0x4000000	// 1600万
...
// /*
					...
					for (; lv5Node != m_lastNode[lv6Id].fnNode; ++lv5Node) {
						...
						if (!(level[lv5Id] & 0x40)) {
							...
							p3List[lv5Id].lens = 100;
							path3Index += 100;		// 预分配为100
							...
							if((p3List[lv5Id].fnList - p3List[lv5Id].stList) == p3List[lv5Id].lens) {	// 如果缓存已满,重新分配
								memcpy(&path3[path3Index], p3List[lv5Id].stList, p3List[lv5Id].lens * sizeof(P3List));
								p3List[lv5Id].stList = &path3[path3Index];
								p3List[lv5Id].fnList = p3List[lv5Id].stList + p3List[lv5Id].lens;
								p3List[lv5Id].lens = p3List[lv5Id].lens << 1;
								path3Index += p3List[lv5Id].lens;
							}
							p3List[lv5Id].fnList->lv6n = lv6Id;
							p3List[lv5Id].fnList->lv6w = lv5Node->weight;
							p3List[lv5Id].fnList++->lv7n = lv7Id;
